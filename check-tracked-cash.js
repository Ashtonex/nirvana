...
async function main() {
  console.log("Checking Tracked Cash components...");
  ...
  // Fetch the data needed for buildCashReconciliation
  const [salesData, ledgerData, opsStateData, investData, shopsData] = await Promise.all([
    ...
    supabaseAdmin.from('invest_deposits').select('*'),
  ...
  // Replicate buildCashReconciliation
  ...
  console.log("\n--- BY SHOP DRAWER DETAILS ---");
  shops.forEach(shop => {
    const sLedger = ledger.filter(e => e.shop_id === shop.id);
    const sSales = sales.filter(s => s.shop_id === shop.id);

    const sOpening = sLedger
      .filter((entry) => entry.category === "Cash Drawer Opening")
      .reduce((sum, entry) => sum + Number(entry.amount || 0), 0);

    const sSalesCash = sSales
      .filter((sale) => (sale.payment_method || sale.paymentMethod) === "cash")
      .reduce((sum, sale) => sum + Number(sale.total_with_tax || 0), 0);

    const sExpenses = sLedger
      .filter((entry) => String(entry.type || "").toLowerCase() === "expense" || isSavingsOrBlackboxTransferEntry(entry))
      .reduce((sum, entry) => sum + Number(entry.amount || 0), 0);

    const sPostedOps = sLedger
      .filter((entry) => entry.category === "Operations Transfer")
      .reduce((sum, entry) => sum + Number(entry.amount || 0), 0);

    const sExpected = sOpening + sSalesCash - sExpenses - sPostedOps;
    console.log(`Shop: ${shop.name} (${shop.id})`);
    console.log(`  - Opening: $${sOpening.toFixed(2)}`);
    console.log(`  - Cash Sales: $${sSalesCash.toFixed(2)}`);
    console.log(`  - Expenses & Savings/Blackbox: $${sExpenses.toFixed(2)}`);
    console.log(`  - Operations Transfer: $${sPostedOps.toFixed(2)}`);
    console.log(`  = Expected Cash: $${sExpected.toFixed(2)}`);
  });
}
